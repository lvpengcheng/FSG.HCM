namespace FSG.HCM.Cli.SourceCode;

public class SourceCodeManager : ITransientDependency, ISourceCodeManager
{
    private readonly ILogger<SourceCodeManager> _logger;
    private readonly IHCMManager _abpProManager;
    private readonly HCMCliOptions _cliOptions;

    public SourceCodeManager(ILogger<SourceCodeManager> logger, IOptions<Options.HCMCliOptions> options, IHCMManager abpProManager)
    {
        _logger = logger;
    ...
    public async Task<TemplateFile> GetAsync(string version)
    {
        ...
        _logger.LogInformation($"FSG HCM Version:{version}");
