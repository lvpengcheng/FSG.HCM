    ...
    where TEntity : class, IElasticSearchEntity
{
    ...
    /// 根据id查询实体
    /// </summary>
    /// <exception cref="HCMElasticSearchEntityNotFoundException"></exception>
    ...
    public virtual async Task<TEntity> GetAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        if (!result.IsValid)
        {
            throw new HCMElasticSearchEntityNotFoundException(innerException: result.OriginalException);
        }
    ...
    /// </summary>
    /// <exception cref="HCMElasticSearchException"></exception>
    public virtual async Task InsertAsync(TEntity entity, CancellationToken cancellationToken = default)
    {
        var result = await Client.IndexAsync(entity, x => x.Index(IndexName), cancellationToken);
        if (!result.IsValid)
        {
            throw new HCMElasticSearchException(innerException: result.OriginalException);
        }
    }
