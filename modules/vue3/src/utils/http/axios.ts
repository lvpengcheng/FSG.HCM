...
baseService.interceptors.request.use(
    config => {
        ...
        //ABP
        const abp_langue = ls.get(ABP_LANGUE)
        const abp_tenantid = ls.get(ABP_TENANTID)
        config.headers['accept-language'] = abp_langue // 让每个请求携带自定义 token 请根据实际情况自行修改
        config.headers['__tenant'] = abp_tenantid // 让每个请求携带自定义 token 请根据实际情况自行修改
        if (token) {
